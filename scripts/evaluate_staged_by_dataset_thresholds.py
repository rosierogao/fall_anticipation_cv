...
from evaluate_six_model_thresholds import MODEL_SPECS, predict_pose_predictive
...
@torch.no_grad()
def predict_pose_predictive_group(df: pd.DataFrame, checkpoint: Path, batch_size: int, device: torch.device):
    # Reuse the complete model-loading path by writing a temporary CSV subset is avoided here
    # by following evaluate_six_model_thresholds' public predictor logic on the caller side.
    from fall_anticipation_cv.models.pose_predictive import PoseSeq2SeqPredictiveModel
    from fall_anticipation_cv.pose_predictive_data import PosePredictiveWindowDataset, collate_pose_predictive_windows
...
def main():
    ...
    parser.add_argument("--thresholds", default="outputs/eight_model_f2_and_balanced_threshold_tuning_metrics.json")
    parser.add_argument("--output", required=True)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--tasks", nargs="+", default=["fall_anticipation", "fallen_state"])
    parser.add_argument("--datasets", nargs="+", default=["GMDCSA24", "le2i", "CAUCAFall"])
    ...
    for spec in MODEL_SPECS:
        if spec["task"] not in args.tasks:
            continue
        ...
        for ds in args.datasets:
            ...
            if spec["kind"] == "pose":
                ...
                labels, probs = predict_pose_predictive_group(group, checkpoint, args.batch_size, device)
            else:
                raise ValueError(spec["kind"])
            item["datasets"][ds] = {}
            for policy, threshold in thresholds[key].items():
                item["datasets"][ds][policy] = add_derived(
                    classification_metrics_from_probs(labels, probs, threshold)
                )
