...
app = modal.App(f"{APP_NAME}-tune-expanded-thresholds")
...
def tune_expanded_thresholds(
    output: str = (
        f"{DATASET_ROOT}/outputs/"
        "expanded_staged_caucafall_oops_f2_and_balanced_threshold_metrics.json"
    ),
    batch_size: int = 32,
    ...
    cmd = [
        sys.executable,
        f"{PACKAGE_REMOTE_ROOT}/scripts/evaluate_expanded_model_thresholds.py",
        "--data-root",
        DATASET_ROOT,
        "--output",
        output,
...
def main(
    ...
    output_path = tune_expanded_thresholds.remote(
        output=output,
        batch_size=batch_size,
        target_recall=target_recall,
    )
    print(f"Saved expanded threshold tuning: {output_path}", flush=True)
