...
app = modal.App(f"{APP_NAME}-staged-by-dataset-thresholds")
...
def evaluate_staged_by_dataset(
    output: str = f"{DATASET_ROOT}/outputs/staged_by_dataset_threshold_metrics.json",
    batch_size: int = 32,
    ...
    cmd = [
        sys.executable,
        f"{PACKAGE_REMOTE_ROOT}/scripts/evaluate_staged_by_dataset_thresholds.py",
        "--data-root",
        DATASET_ROOT,
        "--output",
        output,
...
def main(
    ...
    output_path = evaluate_staged_by_dataset.remote(
        output=output,
        batch_size=batch_size,
        target_recall=target_recall,
        datasets=datasets,
    )
    print(f"Saved staged by-dataset metrics: {output_path}", flush=True)
