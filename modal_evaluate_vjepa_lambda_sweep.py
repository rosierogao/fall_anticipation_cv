...
app = modal.App(f"{APP_NAME}-vjepa-lambda-sweep")
...
@app.function(image=image, volumes={DATA_ROOT: volume}, gpu="H100", timeout=60 * 60 * 2)
def evaluate_lambda_sweep(
    output: str = f"{DATASET_ROOT}/outputs/vjepa_lambda_sweep_threshold_metrics.json",
    batch_size: int = 32,
    ...
    cmd = [
        sys.executable,
        f"{PACKAGE_REMOTE_ROOT}/scripts/evaluate_vjepa_lambda_sweep_thresholds.py",
        "--data-root",
        DATASET_ROOT,
        "--output",
        output,
...
def main(
    ...
    output_path = evaluate_lambda_sweep.remote(
        output=output,
        batch_size=batch_size,
        target_recall=target_recall,
    )
    print(f"Saved lambda sweep threshold metrics: {output_path}", flush=True)
